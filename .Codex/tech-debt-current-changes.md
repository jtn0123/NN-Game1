...
**Validation:** `make check` passed: Black clean, focused mypy clean, 7 dashboard JS tests passed, 537 Python tests passed, total coverage 50.90%.
...
| `src/web/server.py` | 1,950 | Route registration, Socket.IO controls, metrics, and dashboard state in one server class |
| `src/game/space_invaders.py` | 1,976 | Large game object with rendering, collision, wave logic, rewards |
| `tests/test_web_server.py` | 324 | Lower-level server tests remain; route/socket/NN integration tests are now split |
...
| 2 | Architecture | `src/web/server.py` still owns too many concerns | 1,950 lines; `MetricsPublisher` 745 lines; `WebDashboard` 836 lines; `_register_socket_events` complexity reduced to 38 but still high | High | Continue splitting `routes.py`, `socket_controls.py`, and `metrics_publisher.py` |
...
| 6 | Test Quality | Web server test split is healthier, but still needs shared fixtures | `test_web_server.py` is 324 lines; route/socket/NN files are split; fixtures are duplicated | Low | Extract shared dashboard fixtures/builders into `tests/web_dashboard_fixtures.py` |
...
1. Extract shared fixtures for the split web dashboard tests.
   - Effort: 1-2 hours
   - Benefit: Keeps route/socket/NN test modules small without duplicate setup
   - ROI: Medium

2. Extract remaining socket action handling into `socket_controls.py`.
   - Effort: 4-6 hours
   - Benefit: Reduces `_register_socket_events` complexity further and makes launcher/restart controls directly testable
   - ROI: High
...
1. Extract `socket_controls.py` with one function per remaining action.
   - Target: reduce `_register_socket_events` complexity from 38 to under 15.
...
tests/test_web_server.py
  ...
  -> tests/test_web_socket_controls.py
  -> tests/test_web_nn_inspection.py
```
...
```text
src/web/server.py
  -> src/web/socket_controls.py
  -> tests/test_web_socket_controls.py
```

This has moderate UI risk but would further reduce the highest-complexity changed server path.
