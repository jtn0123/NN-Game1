...
### RUN-11 — corrected leg-2 diagnostic (NEAR vs FAR probe) (IN FLIGHT, M4)
Built a FAR probe to fix RUN-10's mismatch: `--leg2-probe` now runs NEAR (next to exit, ~1 tile, the old probe) AND FAR (random reachable standing tile, ~24 tiles, real navigation; oracle-verified the exit is reachable from it). Pushed `e04e872`. Control-only (curriculum disconfirmed), 3 seeds, 4000 ep.
- **Decision tree:** NEAR high & FAR low → wall IS long-range route-to-exit → a corrected far-start curriculum (RUN-12) has upside. NEAR≈FAR high while full-play win ~0.05 → leg-2 NOT the wall → bottleneck is leg-1 collect-all (~0.24) or full-chain credit → ceiling for this lever family. Both low → fundamental (perception/motor/credit) → ceiling.
- This is a DIAGNOSTIC (pinpoints the wall), not a lever. RUN-10's 0.73 NEAR was the trivial hop; FAR is the real test.
