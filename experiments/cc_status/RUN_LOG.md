...
### RUN-15 — difficulty curriculum (warm-start normal ⟵ easy) (BUILT, dispatched to M4)
`--curriculum-easy-episodes` (commit `6a6e4bf`): pretrain at an easier tier, capture weights, warm-start the `--difficulty` run via `prepare_trainer(transfer_weights=...)`. state_size identical across tiers ⇒ strict transfer. Targets RUN-15a's confirmed behavioral completion wall. Both arms keep `--geo-compass`; same 4000-ep budget for fairness.
- A/B: control = normal 4000 ep from scratch · curriculum = 2000 easy → 2000 normal (warm-start). 3 seeds, `--leg2-probe`, graded on held-out **normal**.
- Grade: **exit_unlocked** (does it finally collect ALL crystals? RUN-14 was 0.000) + held-out **win** + **conversion**; crystals must stay ≥0.83.
- Decision: exit_unlocked+wins rise → curriculum cracks completion → RUN-16 finer ramp. Crystals high but exit_unlocked ~0 → last-crystal/hazard-survival tail. No movement → transfer insufficient (longer pretrain / epsilon carry / 3-stage).
- Smoke-tested end-to-end (easy pretrain → normal warm-start → held-out normal eval; weights transfer cleanly).
