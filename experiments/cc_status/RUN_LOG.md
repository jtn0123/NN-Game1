...
### RUN-25 — enemy-motion + win-at-K tier A/B/C/D (M4) — ❌ NO COMPLETION LIFT
Work order from PR #37 at `6764706`: canonical `diagnose_gap --imported`, 4 arms in parallel, 3 seeds, 6000 ep, 48 eval games, checkpoint every 1000, `--save-weights --stall-trace --death-trace --cpu`. Arms: A control, B `--enemy-motion`, C `--win-at-k 15`, D `--enemy-motion --win-at-k 15`. Note: the posted command did not include `--difficulty normal`; the emitted diagnosis records `difficulty=tutorial` with `imported_fixed_set=true`. Ran the command shape exactly as posted. Output: `runs/RUN-25/`.
- **Runtime/preflight:** pulled `6764706`; `pytest tests/test_win_at_k.py tests/test_demo_recorder.py tests/test_crystal_caves_handcrafted.py` passed (`15 passed`); `python -m experiments.cc_status.compass_audit` reported zero trapped/dead/compass-hard-lie cells across all 16 imported levels. Four arm processes completed cleanly; wrapper ended `status=0` at Thu Jul 2 13:11 PDT. Per-milestone weights were saved for all seeds/arms.
- **Result: zero wins everywhere.** No seed/arm/checkpoint produced a nonzero canonical win, so the winner's-curse fresh re-eval guard never triggered. Final/best is ep6000 for all arms:

| arm | config | win | crystal | exit unlock | target | killed | stalled | timeout | enemy | hazard | stall trapped | clock mislabel |
|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| A | control | 0.000 | 0.476 | 0.000 | 0.284 | 0.458 | 0.500 | 0.042 | 0.396 | 0.063 | 0.000 | 0.000 |
| B | enemy motion | 0.000 | 0.489 | 0.000 | 0.282 | 0.500 | 0.438 | 0.063 | 0.417 | 0.083 | 0.000 | 0.000 |
| C | win-at-K 15 | 0.000 | 0.452 | 0.000 | 0.306 | 0.646 | 0.292 | 0.063 | 0.521 | 0.125 | 0.000 | 0.000 |
| D | enemy motion + win-at-K 15 | 0.000 | 0.508 | 0.000 | 0.253 | 0.542 | 0.458 | 0.000 | 0.438 | 0.104 | 0.000 | 0.000 |

- **Curve:** every arm improved crystals late but stayed below RUN-24's long-horizon ~0.60-0.66 crystal band: A 0.240→0.476, B 0.227→0.489, C 0.210→0.452, D 0.226→0.508 from ep1000→6000. Exit unlock stayed **0.000** throughout all aggregate checkpoints.
- **Arm reads:** enemy motion gives a small crystal lift (B +0.013 vs A; D +0.055 vs C) but does not reduce deaths or produce unlocks/wins. Win-at-K alone hurts crystals and increases kills. Combined D is best crystal but worst target progress among the useful arms; it remains collection without objective-chain completion.
- **Instrumentation read:** Phase-0 fixes held. `trapped_frac=0.000` and `clock_mislabel_frac=0.000` for every arm. Prior trapped/clock concerns are not explaining current failures. Stalls are still mostly far from objective (A 0.917, B 0.822, C 0.889, D 1.000 far), with crystals at stall around 0.42-0.50.
- **Verdict:** RUN-25 disconfirms both candidate levers in this exact recipe. Enemy-motion is only a small crystal/coverage tweak, not a completion lever. Win-at-K=15 does not create a useful training tier and appears to trade away competence. The completion wall remains: no exit unlocks and no wins after 3 seeds.
