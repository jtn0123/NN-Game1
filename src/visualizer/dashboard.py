...
class Dashboard:
    ...
    def _draw_performance_zones(
        self, screen: pygame.Surface, rect: pygame.Rect, max_score: float
    ) -> None:
        ...
        for i, (key, value, indicator) in enumerate(metrics):
            ...
            if indicator:
                trend_color = (
                    (100, 220, 130)
                    if indicator == "↑"
                    else (220, 100, 100) if indicator == "↓" else (150, 150, 150)
                )
                if indicator == "★":
    ...
    def _draw_epsilon_gauge(self, screen: pygame.Surface, x: int, y: int, gauge_width: int) -> None:
        ...
        if fill_width > 0:
            ...
            highlight_color = (
                min(255, self.epsilon_color[0] + 40),
        ...
        pct_rect = pct_render.get_rect(
            right=x + gauge_width - 4, centery=gauge_y + gauge_height // 2
        )
