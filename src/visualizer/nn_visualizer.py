...
class NeuralNetVisualizer:
        ...
        for i in range(len(layer_positions) - 1):
            ...
            if i < len(weights):
                ...
                if len(from_indices) * len(to_indices) > max_connections:
                    ...
                    from_sample = list(
                        np.random.choice(
                            from_indices, size=min(6, len(from_indices)), replace=False
                        )
                    )
                    to_sample = list(
                ...
                for fi in from_sample:
                    for ti in to_sample:
                        ...
                        if (
                            fi < from_layer["actual_neurons"]
                            and ti < weight_matrix.shape[0]
                            and len(weight_matrix.shape) > 1
                            and weight_matrix.shape[1] > 0
                        ):
                            ...
                            base_thickness = 2.5 * (
                                self.width / 400.0
                            )  # Scale relative to 400px baseline
    ...
    def _update_pulses(
        self, layer_positions: List[Dict], activations: Dict[str, np.ndarray]
    ) -> None:
    ...
    def _draw_pulses(self, screen: pygame.Surface) -> None:
        ...
        for pulse in self.pulses:
            ...
            color = (
                int(pulse.color[0] * alpha),
                int(pulse.color[1] * alpha),
                int(pulse.color[2] * alpha),
            )
            ...
            bright_color = (
                min(255, int(pulse.color[0] * 1.5)),
                min(255, int(pulse.color[1] * 1.5)),
                min(255, int(pulse.color[2] * 1.5)),
            )
    ...
    def _draw_neurons(
        ...
        for i, (layer_pos, info) in enumerate(zip(layer_positions, layer_info)):
            ...
            for j, pos in enumerate(layer_pos["positions"]):
                ...
                if act_val > 0:
                    color = self._interpolate_color(
                        self.inactive_color, self.color_positive, min(abs(act_val), 1.0)
                    )
                else:
                    color = self._interpolate_color(
                        self.inactive_color, self.color_negative, min(abs(act_val), 1.0)
                    )
                ...
                if self.fast_mode:
                    ...
                    if abs(act_val) > 0.4:
                        ...
                        self._draw_aa_circle(
                            screen, glow_color, (int(pos[0]), int(pos[1])), glow_radius
                        )
    ...
    def _draw_q_values(
        ...
        for i, (q_val, label, icon) in enumerate(
            zip(q_values, self.action_labels, self.action_icons)
        ):
            ...
            self._prev_bar_heights[i] += (
                target_height - self._prev_bar_heights[i]
            ) * self._bar_lerp_speed
            ...
            bar_rect = pygame.Rect(
                int(bar_x), int(qv_y + 50 - bar_height), int(bar_width - 10), bar_height
            )
            ...
            icon_rect = icon_text.get_rect(
                centerx=int(bar_x + bar_width / 2 - 5), top=int(qv_y + 55)
            )
