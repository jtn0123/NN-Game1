...
class ParticleSystem:
    ...
    def get_shake_offset(self) -> Tuple[int, int]:
        ...
        return (int(math.cos(angle) * offset), int(math.sin(angle) * offset))
    ...
    def emit_brick_break(
        self, x: float, y: float, color: Tuple[int, int, int], count: int = 12
    ) -> None:
        """
    ...
    def emit_paddle_hit(
        self, x: float, y: float, color: Tuple[int, int, int], count: int = 5
    ) -> None:
        """
...
def create_radial_gradient(
    radius: int, center_color: Tuple[int, int, int], edge_color: Tuple[int, int, int]
) -> pygame.Surface:
    """
