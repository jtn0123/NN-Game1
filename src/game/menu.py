...
def draw_breakout_icon(
    ...
    for row in range(2):
        for col in range(3):
            ...
            pygame.draw.rect(
                surface, brick_colors[col], (bx, by, brick_w, brick_h), border_radius=2
            )
...
class ActionButton:
    ...
    def __init__(self, text: str, x: int, y: int, key_hint: str = ""):
        ...
        text_surface = font.render(
            f"{key_hint} {text}" if key_hint else text, True, (255, 255, 255)
        )
    ...
    def __init__(self, screen_width: int = 800, screen_height: int = 600):
        ...
        for i in range(8):
            self.diag_lines.append(
                {"offset": i * 150, "speed": 0.3 + i * 0.05, "alpha": 15 + i * 3}
            )
