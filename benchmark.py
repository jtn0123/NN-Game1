...
def run_benchmark(
    ...
    if cfg.device == "cpu":
        ...
        if cfg.device == "mps" and not (
            hasattr(torch.backends, "mps") and torch.backends.mps.is_available()
        ):
            if verbose:
                print("  ⚠️  MPS not available, skipping")
            return None
...
def print_result(result: BenchmarkResult, show_details: bool = True) -> None:
    ...
    print(f"\n  📊 {result.config_name}")
    print(
        f"     Device: {result.device} | Batch: {result.batch_size} | LE: {result.learn_every} | GS: {result.gradient_steps}"
    )
    print(f"     Buffer: {result.buffer_size:,} experiences")
    print(
        f"     Steps: {result.total_steps:,} in {result.total_time}s → {result.steps_per_sec:,.0f} steps/sec"
    )
    print(f"     Gradients: {result.gradient_updates:,} → {result.gradients_per_sec:,.0f} grad/sec")
    if show_details:
        print(
            f"     Episodes: {result.episodes_completed} | Avg length: {result.avg_episode_length:.0f} steps"
        )
...
def print_scaling_analysis(results: List[BenchmarkResult]) -> None:
    ...
    for r in sorted(scaling_results, key=lambda x: x.buffer_size):
        ...
        print(
            f"{r.buffer_size:>12,}   {r.steps_per_sec:>12,.0f} {r.gradients_per_sec:>12,.0f} {delta:>12}"
        )
...
Examples:
  ...
  python benchmark.py --config turbo,realistic_cpu
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Quick 3-second benchmarks with small buffer"
    )
    parser.add_argument("--realistic", action="store_true", help="Test with realistic 50k buffer")
    parser.add_argument(
        "--scaling", action="store_true", help="Test buffer size scaling (1k to 100k)"
    )
    parser.add_argument("--full", action="store_true", help="Full benchmark suite (all configs)")
    parser.add_argument(
        "--duration", type=float, default=5.0, help="Benchmark duration per config (seconds)"
    )
    ...
    if args.list:
        ...
        for name in ["turbo", "cpu_b64", "cpu_b128", "cpu_b256", "cpu_b128_le8", "cpu_b128_le16"]:
            cfg = CONFIGS[name]
            print(
                f"    {name}: device={cfg.device}, batch={cfg.batch_size}, le={cfg.learn_every}, gs={cfg.gradient_steps}"
            )
        ...
        for name in REALISTIC_CONFIGS:
            cfg = CONFIGS[name]
            print(
                f"    {name}: device={cfg.device}, batch={cfg.batch_size}, buffer={cfg.buffer_fill:,}"
            )
    ...
    if results:
        ...
        for r in results:
            ...
            print(
                f"{r.config_name:<20} {r.device:<8} {buf_str:>10} {r.steps_per_sec:>12,.0f} {r.gradients_per_sec:>12,.0f}"
            )
