...
def evaluate_checkpoint(checkpoint_path: str, num_episodes: int = 100, seed: int = 42) -> dict:
    ...
    agent = Agent(state_size=game.state_size, action_size=game.action_size, config=config)
...
def main():
    ...
    parser.add_argument(
        "checkpoints", nargs="*", help="Specific checkpoints to evaluate (e.g., ep29300)"
    )
    ...
    if len(valid_results) >= 2:
        ...
        for r in valid_results:
            s = r["scores"]
            print(
                f"{r['checkpoint']:<35} {s['mean']:>8.1f} {s['median']:>8.1f} {s['max']:>6} {r['win_rate']*100:>5.1f}%"
            )
